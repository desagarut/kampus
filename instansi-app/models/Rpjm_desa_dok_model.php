...
class Rpjm_desa_dok_model extends CI_Model
{
	protected $table = 'rpjm_desa_dok';
	...
	public function get_data($id, string $search = '')
	{
		...
		->from("{$this->table} d")
		->join('rpjm_desa p', 'd.id_rpjm_desa = p.id')
		->where('d.id_rpjm_desa', $id);
	...
	public function insert($id_pembangunan = 0)
	{
		...
		$data['id_rpjm_desa'] = $id_rpjm_desa;
		...
		$outp = $this->db->insert('rpjm_desa_dok', $data);

		if ($outp) $_SESSION['success'] = 1;
		else $_SESSION['success'] = -1;
	}

	public function update($id = 0, $id_rpjm_desa = 0)
	{
		$post = $this->input->post();
		...
		$this->db->where('id', $id);
		$outp = $this->db->update('rpjm_desa_dok', $data);
	...
	public function find_dokumentasi($id_rpjm_desa)
	{
		return $this->db->where('id_rpjm_desa', $id_rpjm_desa)
