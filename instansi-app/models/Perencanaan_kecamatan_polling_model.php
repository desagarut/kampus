...
class Perencanaan_kecamatan_polling_model extends CI_Model
{
	...
	public function get_hasil_polling_kecamatan()
	{
		return $this->db->query("SELECT sum(id_pilihan) AS jml_pilihan, count(id_responden) AS jml_responden, 
		  avg(id_pilihan) AS rata_pilihan from tbl_perencanaan_kecamatan_polling");
	}
	...
	public function insert($id_perencanaan_desa = 0)
	{
		...
		$outp = $this->db->insert('tbl_perencanaan_kecamatan_polling', $data);
	...
	public function update($id = 0, $id_perencanaan_desa = 0)
	{
		...
		$this->db->where('id', $id);
		$outp = $this->db->update('tbl_perencanaan_kecamatan_polling', $data);
